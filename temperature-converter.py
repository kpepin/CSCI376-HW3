...
def convert():
    try: 
        ...
        result_label.classes(remove="text-negative").classes("text-lg font-semibold text-positive mt-4")
        # text-positive: the text is colored in the positive color
    except ValueError:
        result_label.set_text("Please enter a valid number.")
        result_label.classes(remove="text-positive").classes("text-lg font-semibold text-negative mt-4")
...
with ui.row().classes("mx-auto"):
    with ui.card().classes("w-100 p-6 shadow-xl mx-auto mt-10 rounded-xl border").style("border-color: #ff0000;"):
        ...
        input_field = ui.input("Enter Temperature").props('type="number"').classes("w-full mb-4 p-2 text-lg")
        ...
        convert_button = ui.button("Convert", on_click=convert).classes("text-white font-bold py-2 px-4 rounded mx-auto flex justify-center")
        ...
        result_label = ui.label().classes("text-lg mt-4 text-center w-full")
    with ui.card().classes("w-100 p-6 shadow-xl mx-auto mt-10 border rounded-xl").style("border-color: #ff0000;"):
        ui.label("Temperature Converter #2").classes("text-2xl font-bold text-accent mb-4 underline")
        result_label2 = ui.label("50.0°C = 122.0°F").classes("text-lg font-semibold !text-positive mt-4 text-center w-full")
        slider = ui.slider(min=0, max=100, step=1, value=50, on_change=convertv2).props('label="Reverse Slider"').classes("w-full mt-4")
